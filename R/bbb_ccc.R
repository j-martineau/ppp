...
bbb_ccc <- function(x, .bbb, .ccc, ...) {
  ...
  if (base::length(.bbb) != 1) {Errors <- base::c(Errors, ErrBBB)} else if (!(.bbb %in% ppp::bbb_props())) {Errors <- base::c(Errors, ErrBBB)}
  if (base::length(.ccc) != 1) {Errors <- base::c(Errors, ErrCCC)} else if (!(.ccc %in% ppp::ccc_props())) {Errors <- base::c(Errors, ErrCCC)}
